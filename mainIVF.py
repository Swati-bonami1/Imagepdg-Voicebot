...
def image_to_text_gpt4(image_path):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4-turbo",
            messages=[
                {"role": "system", "content": "Analyze the image and provide a detailed description.Identify the main objects, people, animals, text, or any significant elements, explain the overall scene.Explain any actions, emotions, or interactions happening in the image. If applicable, infer the possible context or purpose of the image. Use clear, vivid, and sensory-rich language to make the description engaging."},
                {"role": "user", "content": [
...
def generate_openai_response(query):
    ...
    try:
        vector_db = FAISS.load_local("faiss_index", embeddings=embedding_model, allow_dangerous_deserialization=True)  
...
def upload_file():
    ...
    if pdf_images:
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor: 
            image_texts = list(executor.map(image_to_text_gpt4, pdf_images)) 
            for idx, image_text in enumerate(image_texts):
                pdf_content = [text.replace(f"[IMAGE_{idx}]", image_text if image_text else "[ERROR: Image Processing Failed]") for text in pdf_content]
